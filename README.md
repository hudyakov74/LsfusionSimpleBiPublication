...
## ДОПОЛНИТЕЛЬНО: скрипт для копирования в буфер обмена по нажатию мышкой значений футеров таблиц и сводных таблиц (и значений измерений в сводных - удобно для вставки в фильтр)
...
это небольшой скрипт помогающий генерировать тексты view с наименованиями полей из объектов lsfusion.
Работает если поля названы непосредственно в коде, если там выражения для поддержки мультиязычности то выйдут эти выражения, но можно переименовать.
Полностью базируется на внутреннем механизме reflection самой платформы lsfusion

Поддержка связаных таблиц простая - только как LOOKUP более сложное лучше реализовать на стороне BI системы.
Но lookup на lookup работает - т.е. если в одной вьюхе показали текст из lookup, то следующие вьюхи могут использовать предыдущую.
...
- `schema` — схема базы данных (не используется)
- `comment` — комментарий
