...
на стороне заполняющего (внимание !!! небезопасно: код не проверяется - выполняется в текущей рабочей базе все что там написано)
...
могут быть ошибки - скрипт простой. можно конкретную view поправить руками - сгененировать и поправить руками

![img_8.png](png/img_8.png)

в таблице будет пометка = исправлено вручную
...
создадутся представления и будут доступны из BI систем под выбранным именем и паролем
   (к имени всегда добавляется префикс biuser + login чтобы случайно не повредить системные логины)

 ![img_10.png](png/img_10.png)

имена генерирутся по колонке namе могут быть русскими и с пробелами
их длина ограничена 30 символами т.к. системная длина 64 байта. а русские буквы занимают по 2 байта
